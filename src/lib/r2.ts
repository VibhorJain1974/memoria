...
// ─── CLOUDFLARE R2 ────────────────────────────────────────────
// Primary & only storage. Free 10 GB, then $0.015/GB/month (~₹1.26/GB).
// Admin panel lets you increase the ceiling if you buy more R2 storage.
...
export const DEFAULT_R2_LIMIT  = 10 * 1024 * 1024 * 1024  // 10 GB free tier
...
// smartUpload kept for API compatibility — only R2 now
...
export async function sendStorageAlert(usedBytes: number) {
  ...
  const msg = `Memoria R2: ${usedGB}GB / ${limitGB}GB used. Only ${freeMB}MB left! Go to Admin → Storage to increase limit.`
