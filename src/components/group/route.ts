...
export async function POST(request: NextRequest) {
    try {
        ...
        // Generate a unique invite code (retry up to 5 times)
        ...
        if (memberErr) {
            console.error('Member insert error (non-fatal):', memberErr)
            // Don't fail — group was created, member row is secondary
        }
